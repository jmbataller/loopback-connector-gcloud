...
GcloudDataStoreConnector.prototype.find = function (model, filter, callback) {
   debug('find: %s %s', model, JSON.stringify(JSON.decycle(filter)));
   ...
   var filterClause = function (data, name) {
     ...
     debug('filterClause: %s %s', JSON.stringify(JSON.decycle(data), name);
      ...
      if (idName == name) {
        ...
        debug('filterClause accessType');
         ...
         debug('find: UNSUPPORTED OR CLAUSE %s', JSON.stringify(JSON.decycle(data)));
      }
      else {
         debug('find: adding filter %s = %s', name, JSON.stringify(JSON.decycle(data)));
   ...
   // Show it
   debug('find: execute query %s', JSON.stringify(JSON.decycle(query)));
...
GcloudDataStoreConnector.prototype.create = function (model, data, options, callback) {
   debug('create: %s %s %s', model, JSON.stringify(JSON.decycle(data)), JSON.stringify(JSON.decycle(options)));
   ...
   // Update the data
   debug('create: execute %s', JSON.stringify(JSON.decycle(data)));
...
GcloudDataStoreConnector.prototype.updateAttributes = function (model, id, data, options, callback) {
   debug('updateAttributes: %s %s %s %s', model, id, JSON.stringify(JSON.decycle(data)), JSON.stringify(JSON.decycle(options)));
   ...
   this.findById(model, id, function (errors, original) {
      ...
      // Showcase
      debug('updateAttributes: execute %s', JSON.stringify(JSON.decycle(data)));
      ...
      ds.save({
         ...
         debug('updateAttributes: result %s', JSON.stringify(JSON.decycle(errors || result)));
...
GcloudDataStoreConnector.prototype.destroyAll = function (model, where, options, callback) {
   debug('destroyAll: %s %s %s', model, JSON.stringify(JSON.decycle(where)), JSON.stringify(JSON.decycle(options)));
   ...
   ds.delete(key, function (errors, result) {
      debug('destroyAll: result %s', JSON.stringify(JSON.decycle(errors || result)));
...
GcloudDataStoreConnector.prototype.destroy = function (model, id, options, callback) {
   debug('destroy: %s %s %s', model, JSON.stringify(id), JSON.stringify(JSON.decycle((options))));
   ...
   ds.delete(key, function (errors, result) {
      debug('destroy: result %s', JSON.stringify(JSON.decycle(errors || result)));
      if (callback === null || callback === undefined) {
         return;
      }
...
GcloudDataStoreConnector.prototype.count = function (model, where, options, callback) {
   debug('count: %s %s %s', model, JSON.stringify(JSON.decycle(where), JSON.stringify(JSON.decycle(options))));
   ...
   return this.find(model, {
      ...
      debug('count: result %s', JSON.stringify(JSON.decycle(errors || result)));
