...
GcloudDataStoreConnector.prototype.find = function (model, filter, callback) {
   debug('find: %s %s', model, stringify(filter));
   ...
   var filterClause = function (data, name) {
     ...
     debug('filterClause: %s %s', stringify(data), name);
      ...
      if (idName == name) {
        ...
        debug('filterClause accessType');
         ...
         debug('find: UNSUPPORTED OR CLAUSE %s', stringify(data));
      }
      else {
         debug('find: adding filter %s = %s', name, stringify(data));
   ...
   // Show it
   debug('find: execute query %s', stringify(query));

   // Run the query
   ds.runQuery(query, function (errors, result, info) {
      debug('find: results %s', stringify(errors || result));
...
GcloudDataStoreConnector.prototype.create = function (model, data, options, callback) {
   debug('create: %s %s %s', model, stringify(data), stringify(options));
   ...
   // Update the data
   debug('create: execute %s', stringify(data));

   ds.save({
      key : key,
      data: data
   }, function (errors, result) {
      debug('create: result %s', stringify(errors || result));
...
GcloudDataStoreConnector.prototype.updateAttributes = function (model, id, data, options, callback) {
   debug('updateAttributes: %s %s %s %s', model, id, stringify(data), stringify(options));
   ...
   this.findById(model, id, function (errors, original) {
      ...
      // Showcase
      debug('updateAttributes: execute %s', stringify(data));
      ...
      ds.save({
         ...
         debug('updateAttributes: result %s', stringify(errors || result));
...
GcloudDataStoreConnector.prototype.destroyAll = function (model, where, options, callback) {
   debug('destroyAll: %s %s %s', model, stringify(where), stringify(options));
   ...
   // Update the data
   debug('destroyAll: execute %s', stringify(key));

   ds.delete(key, function (errors, result) {
      debug('destroyAll: result %s', stringify(errors || result));
...
GcloudDataStoreConnector.prototype.destroy = function (model, id, options, callback) {
   debug('destroy: %s %s %s', model, stringify(id), stringify(options));
   ...
   // Update the data
   debug('destroy: execute %s', stringify(key));

   ds.delete(key, function (errors, result) {
      debug('destroy: result %s', stringify(errors || result));
      if (callback === null || callback === undefined) {
         return;
      }
...
GcloudDataStoreConnector.prototype.count = function (model, where, options, callback) {
   debug('count: %s %s %s', model, stringify(where), stringify(options));
   ...
   return this.find(model, {
      ...
      debug('count: result %s', stringify(errors || result));
